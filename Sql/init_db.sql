...
CREATE TABLE `stock_date`  (
  ...
  `JinShou` float(8, 2) NOT NULL COMMENT '今收',
  `ZuoShou` float(8, 2) DEFAULT NULL COMMENT '昨收',
  `ShangZhang` float(6, 2) NOT NULL COMMENT '上涨（价格）',
  `ZhangFu` float(4, 2) DEFAULT NULL COMMENT '涨幅百分点(如1.58代表上涨1.58%)',
  `JinKai` float(8, 2) NOT NULL COMMENT '今开',
  `HuanSouLv` float(6, 2) NOT NULL COMMENT '换手率百分点',
  `ChengJiaoLiang` int(20) NOT NULL COMMENT '成交量(手)',
  `ChengJiaoE` bigint(40) NOT NULL COMMENT '成交额(万元)',
  `ZuiGao` float(8, 2) NOT NULL COMMENT '当日最高价',
  `ZuiDi` float(8, 2) NOT NULL COMMENT '当日最低价',
  `ZhenFu` float(4, 2) NOT NULL COMMENT '当日振幅',
  `NeiPan` int(12) NOT NULL COMMENT '内盘（手）',
  `WaiPan` int(12) NOT NULL COMMENT '外盘（手）',
  ...
  `ShiYingLv_MRQ` float(10, 2) NOT NULL COMMENT '市盈率',
  `ShiJingLv` float(10, 2) NOT NULL COMMENT '市净率',
